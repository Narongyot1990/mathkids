...
/// New Subtraction Game Widget - Visual subtraction with taking away animations
class SubtractionGameWidget extends BaseGameWidget {
  ...
  Widget _buildVisualSubtraction(BuildContext context, int operand1, int operand2, double iconSize) {
    final emoji = GameEmojis.counting.getByIndex(operand1 + operand2);
    
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        // Initial group - operand2 items to be taken away
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            // First number (total)
            ...
            _buildSubtractBox(operand2, iconSize),
        ...
        // Explanation
        if (operand2 > 0)
          _buildExplanation(operand1, operand2, iconSize),
      ],
    );
  }
  ...
  Widget _buildEmojiGroup({
    ...
    required int showCrossedOut,
  }) {
    final remaining = count - showCrossedOut;
    ...
    return Column(
      children: [
        ...
        // Emojis grid with crossed out animation
        Container(
          ...
          child: Wrap(
            ...
            children: List.generate(count, (index) {
              final isCrossedOut = index >= remaining;
              ...
              return TweenAnimationBuilder<double>(
                ...
                builder: (context, value, child) {
                  return Transform.scale(
                    scale: value,
                    child: Opacity(
                      opacity: isCrossedOut ? 0.3 : value,
                      child: Stack(
                        children: [
                          Text(
                            emoji,
                            style: TextStyle(
                              fontSize: size * 0.7,
                              color: isCrossedOut ? Colors.grey : null,
                            ),
                          ),
                          if (isCrossedOut && isCorrect != null)
                            Positioned.fill(
                              child: Icon(
                                Icons.close,
  ...
  Widget _buildSubtractBox(int number, double size) {
    return Container(
      width: size * 0.9,
      height: size * 0.9,
      decoration: BoxDecoration(
  ...
  Widget _buildExplanation(int operand1, int operand2, double size) {
    final remaining = operand1 - operand2;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            'ลบ $operand2 ออก เหลือ $remaining',
            style: TextStyle(
              fontSize: size * 0.25,
